...
# Clean Education names
pdf["EDUCATION_LEVELS_NAME"] = pdf["EDUCATION_LEVELS_NAME"].apply(lambda x: re.sub(r"[^\x00-\x7F]+", "", x))
...
df = df.withColumn(
  "REMOTE_GROUP",
  when(col("REMOTE_TYPE_").rlike("|".join([f"(?i){deg}" for deg in lower_deg])), "Bachelor's or lower")
  .when(col("EDUCATION_LEVELS_NAME").rlike("|".join([f"(?i){deg}" for deg in higher_deg])), "Master's or PhD")
  .otherwise("Other")
)
